...
export default function ProductDetailPage({
  ...
  useEffect(() => {
    ...
    // Khởi tạo kết nối SSE tới kênh live của sản phẩm
    const eventSource = new EventSource('/api/products/' + id + '/live');

    // Khi nhận được sự kiện onmessage, lập tức cập nhật lại state product
    eventSource.onmessage = (event) => {
      try {
        const data = JSON.parse(event.data);
        setProduct((prev) => {
          if (!prev) return null;
          // Cập nhật giá hiện tại và thời gian kết thúc đấu giá trên giao diện
          return {
            ...prev,
